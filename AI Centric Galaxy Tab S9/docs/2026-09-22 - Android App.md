...
- Downloads accept only the same-origin export endpoint, allowed formats and bounded size/time; saving uses ACTION_CREATE_DOCUMENT. Uploads use ACTION_OPEN_DOCUMENT.
...
`./galaxy android-build` assembles both APKs, runs seven launcher/transfer-policy unit tests and Android lint. It is bounded to ten minutes with one Gradle worker. AGP 8.12.0, Gradle 8.13 with checksum verification, Java 17. SDK, builds, signing keys and APKs are ignored.
